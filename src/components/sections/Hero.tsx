  ...
  return (
    <section className="relative py-20 overflow-hidden">
      ...
      <div 
        ...
        style={{
          background: "radial-gradient(1000px 500px at 30% 20%, rgba(99,102,241,0.15), transparent 60%), radial-gradient(900px 400px at 70% 80%, rgba(168,85,247,0.12), transparent 60%)"
        }}
      />
          ...
          <div className="space-y-8 animate-slide-up">
            <div className="space-y-6">
              ...
              <p className="text-xl text-muted-foreground max-w-2xl leading-relaxed">
                Tire dúvidas em tempo real, <strong className="text-foreground">marque e confirme consultas</strong>, 
                <strong className="text-foreground"> qualifique leads</strong> e integre com WhatsApp, Instagram e Facebook. 
                Também criamos <strong className="text-foreground">Landing Pages</strong> que direcionam para as redes 
                e aumentam a conversão.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <OutlineButton 
                size="lg"
                className="text-lg px-8 py-3"
                onClick={() => window.open(whatsappLink("Olá, vi a demo no hero da agentiss.shop e quero testar agora. Objetivo: marcar consultas/qualificar leads. Negócio: [digite]."), '_blank')}
              >
                Pedir Demo
              </OutlineButton>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 max-w-md">
              <Card className="p-4 bg-card/50 backdrop-blur-sm border-border/50">
                <div className="text-sm text-muted-foreground">Tempo economizado (média)</div>
                <div className="text-2xl font-bold text-primary">~12h</div>
                <div className="text-xs text-muted-foreground">por semana</div>
              </Card>
              <Card className="p-4 bg-card/50 backdrop-blur-sm border-border/50">
                <div className="text-sm text-muted-foreground">SLA primeira resposta</div>
...
function ChatMockup() {
  return (
    <Card className="relative bg-gradient-card backdrop-blur-xl border-border/20 p-6 shadow-card hover:shadow-glow transition-all duration-500 transform hover:scale-[1.02] animate-float">
      {/* Header */}
      ...
      <div className="py-4 space-y-3 max-h-80 overflow-y-auto">
        ...
        <AgentMessage>
          Olá! Posso agendar para <strong>15:30</strong> ou <strong>17:00</strong>. 
          Prefere qual horário? Posso confirmar seus dados.
        </AgentMessage>
        ...
        <AudioMessage duration="0:05" text="Consulta confirmada às 17:00. Até breve!" />
      </div>
...
function AudioMessage({ isUser = false, duration, text }: { isUser?: boolean; duration: string; text?: string }) {
  return (
    <div className={`flex gap-2 items-start ${isUser ? '' : 'justify-end'}`}>
      {isUser && <div className="w-7 h-7 rounded-full bg-muted/30 border border-muted/50" />}
      <div className={`max-w-[80%] ${isUser ? 'bg-muted/20 border-muted/30' : 'bg-success/20 border-success/30'} border rounded-2xl px-3 py-2`}>
        <div className="flex items-center gap-2 text-sm">
          <div className="w-4 h-4 text-current">🎵</div>
          <div className="flex gap-1">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="w-1 h-3 bg-current/60 rounded animate-pulse" style={{ animationDelay: `${i * 0.1}s` }} />
            ))}
          </div>
      ...
      {!isUser && <div className="w-7 h-7 rounded-full bg-success/30 border border-success/50" />}
    </div>
  );
}
...
function MetricCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-background/20 backdrop-blur-sm border border-border/30 rounded-lg p-2 text-center">
